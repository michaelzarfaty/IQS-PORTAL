...
const Onboarding = () => {
  ...
  const [onboardingType, setOnboardingType] = useState<"Qualifier" | "Client" | "Vendor">("Qualifier");
  ...
  const vendorSteps = [
    { id: 1, title: "Vendor Profile", icon: <UserSquare2 className="w-5 h-5" />, desc: "Service offerings and regions" },
    { id: 2, title: "Credential Upload", icon: <ShieldCheck className="w-5 h-5" />, desc: "Upload licenses and insurance" },
    { id: 3, title: "Network Agreement", icon: <FileText className="w-5 h-5" />, desc: "Sign vendor terms" },
    { id: 4, title: "Admin Review", icon: <ShieldCheck className="w-5 h-5" />, desc: "Awaiting admin approval" },
    { id: 5, title: "Active Vendor", icon: <CheckCircle2 className="w-5 h-5" />, desc: "Ready to accept work orders" }
  ];

  const steps = onboardingType === "Qualifier" ? qualifierSteps : onboardingType === "Client" ? clientSteps : vendorSteps;
          ...
          <>
            <div className="bg-white rounded-xl p-6 shadow-sm border border-border/50 text-center">
              <div className="flex justify-center gap-4 mb-4">
                ...
                <Badge 
                  className={`cursor-pointer ${onboardingType === 'Vendor' ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'} hover:bg-primary/80 border-none`}
                  onClick={() => { setOnboardingType('Vendor'); setCurrentStep(1); }}
                >
                  Vendor Onboarding
                </Badge>
              </div>
          ...
          <div className="min-h-[300px] flex flex-col justify-center items-center text-center border-2 border-dashed border-border/50 rounded-xl p-8 bg-muted/10">
            {currentStep === 1 && (
              <div className="max-w-md space-y-4 animate-in fade-in zoom-in-95">
                ...
                <h2 className="text-2xl font-bold">{onboardingType === 'Qualifier' ? 'Complete Your Profile' : onboardingType === 'Client' ? 'Company Profile' : 'Vendor Profile'}</h2>
                <p className="text-muted-foreground">{onboardingType === 'Qualifier' ? 'Please provide your full contact information, service areas, and primary trades.' : onboardingType === 'Client' ? 'Enter your business details, EIN, and primary hiring needs.' : 'Provide your service offerings, regions covered, and contact details.'}</p>
            ...
            {currentStep === 2 && (
              <div className="max-w-md space-y-4 animate-in fade-in zoom-in-95 w-full">
                ...
                <h2 className="text-2xl font-bold">{onboardingType === 'Qualifier' ? 'Verify Your Licenses' : onboardingType === 'Client' ? 'Payment Setup' : 'Credential Upload'}</h2>
                <p className="text-muted-foreground">{onboardingType === 'Qualifier' ? 'Upload copies of your active state licenses.' : onboardingType === 'Client' ? 'Connect your preferred payment processor for vendor fees.' : 'Upload your vendor licenses and insurance certificates.'}</p>
                ...
                <div className="border-2 border-dashed border-border/60 rounded-lg p-6 bg-white hover:bg-muted/10 transition-colors cursor-pointer flex flex-col items-center justify-center text-center group mt-4">
                  <div className="w-10 h-10 bg-primary/10 text-primary rounded-full flex items-center justify-center mb-2 group-hover:scale-110 transition-transform">
                    {onboardingType === 'Client' ? <CreditCard className="w-5 h-5" /> : <FileText className="w-5 h-5" />}
                  </div>
                  <p className="font-medium text-sm">{onboardingType === 'Client' ? 'Connect Stripe Account' : 'Click to upload or drag and drop'}</p>
                ...
                <div className="flex gap-2 mt-4">
                  ...
                  <Button className="w-full" onClick={() => setCurrentStep(3)}>{onboardingType === 'Qualifier' ? 'Verify Licenses' : onboardingType === 'Vendor' ? 'Upload Credentials' : 'Complete Setup'} <ArrowRight className="w-4 h-4 ml-2" /></Button>
                </div>
              </div>
            )}
            {currentStep === 3 && (
              <div className="max-w-md space-y-4 animate-in fade-in zoom-in-95">
                ...
                <p className="text-muted-foreground">Review and electronically sign the {onboardingType === 'Qualifier' ? 'master qualifier agreement' : onboardingType === 'Client' ? 'client service agreement' : 'network vendor agreement'} and waivers.</p>
                <div className="flex gap-2 mt-4">
